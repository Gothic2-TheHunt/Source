...
func int B_TeachPlayerTalentTakeAnimalTrophy(var C_Npc slf,var C_Npc oth,var int trophy)
{
	...
	var int knownDragonTalents;
	knownTalents = 0;
	knownDragonTalents = 0;
	if(PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_Teeth] == TRUE)
	{
		knownTalents = knownTalents + 1;
	};
	...
	{
		knownDragonTalents = knownDragonTalents + 1;
	};
	...
	if(oth.guild == GIL_DJG)
	{
		Npc_SetTalentSkill(oth,NPC_TALENT_TAKEANIMALTROPHY,knownTalents + ((knownDragonTalents + 1) * 13));
	}
	else
	{
		Npc_SetTalentSkill(oth,NPC_TALENT_TAKEANIMALTROPHY,knownTalents);
	};
