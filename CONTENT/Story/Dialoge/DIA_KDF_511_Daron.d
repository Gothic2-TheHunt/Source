...
func void B_DaronSegen()
{
	...
	else if((Daron_Spende >= 750) && (Daron_Spende < 1000) && (Bonus_4 == FALSE))
	{
		other.lp = other.lp + 2;
...
func int DIA_Addon_Daron_GuildHelp_Condition()
{
	if((MIS_Addon_Vatras_Go2Daron == LOG_Running) &&
		...
		(MIS_Addon_Daron_GetStatue != LOG_FAILED) && 
		(MIS_Addon_Daron_GetStatue != LOG_OBSOLETE))
	{
		return TRUE;
	};
};
...
func void DIA_Addon_Daron_FoundStatue_Info()
{
	...
	BroughtStatueToDaron = TRUE;
	B_GivePlayerXP(XP_ShowedDaronStatue);
};
...
func void DIA_Addon_Daron_Spende_wo()
{
	AI_Output(other,self,"DIA_Addon_Daron_GuildHelp_wo_15_00");	//Where was it that you lost that statuette?
	AI_Output(self,other,"DIA_Addon_Daron_GuildHelp_wo_10_01");	//I was on my way to the monastery. Near Orlan's tavern.
	Info_AddChoice(DIA_Daron_Spende,"I've heard enough. I'll find the thing for you.",DIA_Addon_Daron_GuildHelp_auftrag);
	Info_AddChoice(DIA_Daron_Spende,"Orlan's tavern? Where would that be?",DIA_Addon_Daron_GuildHelp_woTaverne);
};
...
func void DIA_Daron_Spende_Info()
{
	...
	if(MIS_Addon_Daron_GetStatue != LOG_SUCCESS && BroughtStatueToDaron == FALSE)
	{
		...
		{
			MIS_Addon_Daron_GetStatue = LOG_Running;
			Info_AddChoice(DIA_Daron_Spende,"You said that the goblins have it now?",DIA_Addon_Daron_GuildHelp_gobbos);
			Info_AddChoice(DIA_Daron_Spende,"Where was it that you lost that statuette?",DIA_Addon_Daron_Spende_wo);
			Info_AddChoice(DIA_Daron_Spende,"Didn't you try to get that statuette back?",DIA_Addon_Daron_GuildHelp_wiederholen);
		};
	}
	else if(Daron_Spende < 1000)
	{
