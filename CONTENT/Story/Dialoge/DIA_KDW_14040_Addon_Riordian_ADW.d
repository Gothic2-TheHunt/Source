...
func void DIA_Riordian_ADW_TeachAlchemy_Info()
{
	...
	if((PLAYER_TALENT_ALCHEMY[POTION_Perm_Mana] == FALSE) && (PLAYER_TALENT_ALCHEMY[POTION_Mana_03] == TRUE) && (Kapitel >= 2))
	{
	...
	if((PLAYER_TALENT_ALCHEMY[POTION_Perm_DEX] == FALSE) && (Kapitel >= 3))
	{
...
instance DIA_Riordian_ADW_PICKPOCKET(C_Info)
{
	npc = KDW_14040_Addon_Riordian_ADW;
	nr = 900;
	condition = DIA_Riordian_ADW_PICKPOCKET_Condition;
	information = DIA_Riordian_ADW_PICKPOCKET_Info;
	permanent = TRUE;
	description = "(It would be damn difficult to steal his potion)";
};


func int DIA_Riordian_ADW_PICKPOCKET_Condition()
{
	if((Npc_GetTalentSkill(other,NPC_TALENT_PICKPOCKET) == 1) && (self.aivar[AIV_PlayerHasPickedMyPocket] == FALSE) && (other.attribute[ATR_DEXTERITY] >= (86 - Theftdiff)))
	{
		return TRUE;
	};
};

func void DIA_Riordian_ADW_PICKPOCKET_Info()
{
	Info_ClearChoices(DIA_Riordian_ADW_PICKPOCKET);
	Info_AddChoice(DIA_Riordian_ADW_PICKPOCKET,Dialog_Back,DIA_Riordian_ADW_PICKPOCKET_BACK);
	Info_AddChoice(DIA_Riordian_ADW_PICKPOCKET,DIALOG_PICKPOCKET,DIA_Riordian_ADW_PICKPOCKET_DoIt);
};

func void DIA_Riordian_ADW_PICKPOCKET_DoIt()
{
	if(other.attribute[ATR_DEXTERITY] >= 86)
	{
...
func void DIA_Riordian_ADW_PICKPOCKET_BACK()
{
	Info_ClearChoices(DIA_Riordian_ADW_PICKPOCKET);
};
