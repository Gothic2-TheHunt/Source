...
func void B_GiveTradeInv_Addon_Martin(var C_Npc slf)
{
	if((Kapitel >= 1) && (Martin_ItemsGiven_Chapter_1 == FALSE))
	{
		...
		CreateInvItems(slf,ItMw_ShortSword1,2);
	...
	if((Kapitel >= 1) && (Martin_ItemsGiven_JoinGuild == FALSE) && hero.guild != GIL_NONE)
	{
		CreateInvItems(slf,ItMw_1H_Common_01,2);
		CreateInvItems(slf,ItSc_PalHolyBolt,1);
		CreateInvItems(slf,ItSc_PalRepelEvil,1);
		CreateInvItems(slf,ItSc_PalMediumHeal,1);
		CreateInvItems(slf,ItMi_Moleratlubric_MIS,1);
		Martin_ItemsGiven_JoinGuild = TRUE;
	};
	if((Kapitel >= 2) && (Martin_ItemsGiven_Chapter_2 == FALSE))
	{
		...
		CreateInvItems(slf,ItSc_PalMediumHeal,1);
		CreateInvItems(slf,ItSc_PalRepelEvil,1);
		CreateInvItems(slf,ItMw_Addon_PIR1hSword,2);
	...
	if((Kapitel >= 3) && (Martin_ItemsGiven_Chapter_3 == FALSE))
	{
		...
		CreateInvItems(slf,ItMW_Addon_Hacker_2h_01,2);
		CreateInvItems(slf,ItMi_Nugget,1);
		CreateInvItems(slf,ItMw_Runenschwert,2);
		CreateInvItems(slf,ItMw_Meisterdegen,2);
