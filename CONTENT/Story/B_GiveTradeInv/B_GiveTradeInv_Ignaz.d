...
var int Ignaz_ItemsGiven_JoinGuild;
...
func void B_GiveTradeInv_Ignaz(var C_Npc slf)
{
	...
	if((Kapitel >= 1) && (Ignaz_ItemsGiven_JoinGuild == FALSE) && hero.guild != GIL_NONE)
	{
		CreateInvItems(slf,ItMi_Gold,100);
		...
		CreateInvItems(slf,ItMi_Flask,12);
		CreateInvItems(slf,ItSc_TrfSheep,1);
		CreateInvItems(slf,ItSc_TrfGiantBug,1);
		...
		CreateInvItems(slf,ItSc_Charm,3);
		CreateInvItems(slf,ItSc_Sleep,1);
		Ignaz_ItemsGiven_JoinGuild = TRUE;
	};
	if((Kapitel >= 2) && (Ignaz_ItemsGiven_Chapter_2 == FALSE))
	{
		...
		CreateInvItems(slf,ItMi_Flask,40);
