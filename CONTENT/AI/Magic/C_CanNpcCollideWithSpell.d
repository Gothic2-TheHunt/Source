...
func int C_CanNpcCollideWithSpell(var int spellType)
{
	...
	{
		if(C_NpcIsDown(self) || C_BodyStateContains(self,BS_SWIM) || C_BodyStateContains(self,BS_DIVE) || (self.guild == GIL_SUMMONED_GOLEM) || (self.guild == GIL_SUMMONED_DEMON) || (self.guild == GIL_STONEGOLEM) || (self.guild == GIL_FIREGOLEM) || (self.guild == GIL_ICEGOLEM) || (self.guild == GIL_TROLL))
		{
			return COLL_DONOTHING;
		};
		if(self.guild == GIL_SWAMPGOLEM)
		{
			return COLL_APPLYHALVEDAMAGE;
		};
	...
	if(spellType == SPL_WaterFist)
	{
		...
		if((self.guild == GIL_SWAMPSHARK) || (self.guild == GIL_LURKER))
		{
			return COLL_APPLYHALVEDAMAGE;
		};
	...
	{
		if(Npc_GetDistToNpc(other,self) >= 1250)
		{
			return COLL_DONOTHING;
		};
		if((self.guild == GIL_SUMMONED_GOLEM) || (self.guild == GIL_SUMMONED_DEMON) || (self.guild == GIL_STONEGOLEM) || (self.guild == GIL_FIREGOLEM) || (self.guild == GIL_ICEGOLEM) || (self.guild == GIL_TROLL))
		{
			return COLL_DONOTHING;
		};
		if(self.guild == GIL_SWAMPGOLEM)
	...
	if(spellType == SPL_PalHolyBolt)
	{
		if(C_NpcIsEvil(self))
		{
			if(C_NpcIsUndead(self) && (self.aivar[AIV_MM_REAL_ID] != ID_UNDEADLORD))
			{
				return COLL_APPLYDOUBLEDAMAGE;
			};
