...
func void Use_XardasSeamapBook_Mis()
{
	...
	{
		Doc_PrintLines(nDocID,0,"... The signs are unmistakable! When the One comes, he will need all the help we can give him. Beliar is already too strong. I have succeeded in obtaining some valuable artifacts which I will keep down here just in case. I suspect the Chosen One will come from among our ranks,");
		Doc_SetMargins(nDocID,-1,30,20,275,20,1);
		Doc_SetFont(nDocID,1,FONT_BookHeadline);
		Doc_SetFont(nDocID,1,FONT_Book);
...
func void UseItPo_PotionOfDeath()
{
	if(hero.guild == GIL_KDF)
	{
		...
		logBonus = ConcatStrings(logBonus, " strength from drinking The Tears of Innos");
		Log_CreateTopic(Topic_Strength,LOG_NOTE);
		B_LogEntry(Topic_Strength,logBonus);
		var string logBonus2;
		logBonus2 = ConcatStrings("I have now gained a total of ", IntToString(5));
		logBonus2 = ConcatStrings(logBonus2, " dexterity from drinking The Tears of Innos");
		Log_CreateTopic(Topic_Dexterity,LOG_NOTE);
		B_LogEntry(Topic_Dexterity,logBonus2);
		var string logBonus3;
		logBonus3 = ConcatStrings("I have now gained a total of ", IntToString(10));
		logBonus3 = ConcatStrings(logBonus3, " mana from drinking The Tears of Innos");
		Log_CreateTopic(Topic_Mana,LOG_NOTE);
		B_LogEntry(Topic_Mana,logBonus3);
